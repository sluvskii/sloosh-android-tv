...
class HlsProxyServer(
    ...
    private var connectionPool = ConnectionPool(10, 30, TimeUnit.SECONDS)
    ...
    private val recentSegments = java.util.concurrent.CopyOnWriteArrayList<String>()
        ...
        try {
            socket.use {
                ...
                if (path.startsWith("/sub/") && path.endsWith(".m3u8")) {
                    ...
                    val track = if (idx != null) subtitleTracks.getOrNull(idx) else null
                    if (track != null) {
                        serveVttPlaylist(track.third, output)
                        return@use
                    }
                }
    ...
    private suspend fun serveVttPlaylist(vttUrl: String, out: OutputStream) {
        val body = fetchText(vttUrl) ?: run { send404(out); return }
                    ...
                    if (line.contains("URI=")) {
                        line.replace(Regex("""URI="([^"]+)"""")) { match ->
                            ...
                            if (uri.isBlank() || uri == "none") match.value
                                ...
                                """URI="${proxyUrl(absolute)}""""
                            }
                        }
                    } else line
                }
                else -> {
                    val absolute = if (line.startsWith("http")) line else base + line
                    segUrls.add(absolute)
                    proxyUrl(absolute)
                }
            }
        }
    ...
    private suspend fun fetchSegmentFromFreshPlaylist(failedUrl: String): ByteArray? {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (bytes != null) {
                Log.d(TAG, "Recovered $segName after session refresh")
                synchronized(cacheLock) { cache[failedUrl] = bytes }
                return bytes
            }
            Log.w(TAG, "New session also 403 for $segName -- triggering forced restart")
            scope.launch(Dispatchers.Main) { onSessionExpired() }
        }
        Log.w(TAG, "Failed to recover $segName within timeout")
        return null
    }
    ...
    private fun fetchBytes(url: String, retries: Int = 2): ByteArray? {
        for (i in 0..retries) {
            try {
                ...
                Log.w(TAG, "fetchBytes attempt $i error: ${e.message}")
            }
            if (i < retries) {
